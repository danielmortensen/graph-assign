...
Take any $s$-$t$ cut $(S, T)$ in an arbitrary directed graph.
The edges crossing from $S$ to $T$ form a bipartite structure:
each has one endpoint in $S$ and one in $T$.  Every unit of flow
from $s$ to $t$ must cross this interface.
...
\begin{tikzpicture}[every node/.style={circle, draw, minimum size=6mm,
  ...
  \draw[->, very thick, blue!70!black] (u)--(y);
  \draw[->, very thick, blue!70!black] (v)--(y);
  % Internal T edges (gray)
  \draw[->, gray] (x)--(t);
  \draw[->, gray] (y)--(t);
  \draw[->, gray] (x)--(y);
  % Region outlines
  \draw[dashed, rounded corners] (-0.7,-1.5) rectangle (2.3,1.6);
  \draw[dashed, rounded corners] (3.7,-1.5) rectangle (6.7,1.6);
  \node[draw=none, font=\small] at (0.8,1.9) {$S$};
  \node[draw=none, font=\small] at (5.2,1.9) {$T$};
\end{tikzpicture}
\end{center}

The bold edges are the bipartite crossing; the gray edges are
internal plumbing.
...
\begin{tikzpicture}[every node/.style={circle, draw, minimum size=6mm,
  ...
  \node (y) at (4.2,-0.8) {$y$};
  \node[fill=red!20] (t) at (6,0) {$t$};
  % Narrow internal edges (the real bottleneck)
  \draw[->, very thick, orange] (s) -- node[above, draw=none,
    font=\scriptsize]{2} (u);
  \draw[->, very thick, orange] (s) -- node[below, draw=none,
    font=\scriptsize]{1} (v);
  % Wide cut edges
  \draw[->] (u) -- node[above, draw=none, font=\scriptsize]{10} (x);
  \draw[->] (v) -- node[below, draw=none, font=\scriptsize]{10} (y);
  \draw[->, gray] (x)--(t);
  \draw[->, gray] (y)--(t);
  % Wider cut (blue, dashed)
  \draw[dashed, rounded corners, blue!50, thick]
    (-0.7,-1.5) rectangle (2.6,1.6);
  \node[draw=none, font=\small, blue!50] at (0.95,1.9)
    {$S$: cap $= 20$};
  % Tighter cut (orange, dashed)
  \draw[dashed, rounded corners, orange, thick]
    (-0.5,-1.3) rectangle (0.5,1.4);
  \node[draw=none, font=\small, orange] at (0,-1.6)
    {$S'$: cap $= 3$};
\end{tikzpicture}
\end{center}

The cut $(S, T)$ has capacity~20 at the bipartite crossing, but
the edges leaving $s$ can carry only~3.  The tighter cut
$S' = \{s\}$ exposes the real bottleneck with capacity~3.
